...
// Archivo de configuración de versión (actualizado automáticamente por deploy-apk.ps1)
const VERSION_FILE = path.join(__dirname, "../version.json");

// Versión por defecto (fallback si no existe version.json)
const DEFAULT_VERSION = {
  version: "1.0.2",
  buildNumber: 2,
  releaseDate: "2026-03-04T00:00:00.000Z",
  releaseNotes: "Sistema de sociedades y verificación SAT implementados",
...
/**
 * Lee la configuración de versión desde version.json
 * Si no existe, usa la versión por defecto
 */
async function getVersionInfo() {
  try {
    const versionData = await fs.readFile(VERSION_FILE, "utf8");
    const versionConfig = JSON.parse(versionData);
    ...
    console.log("⚠️ version.json no encontrado, usando versión por defecto");
...
router.get("/version", async (req, res) => {
  try {
    ...
    // Leer configuración de versión
    const versionConfig = await getVersionInfo();

    // Construir URL completa de descarga
    const baseUrl = `${req.protocol}://${req.get("host")}`;
    const versionInfo = {
      ...versionConfig,
      downloadUrl: `${baseUrl}${versionConfig.downloadUrl}`,
    };

    console.log(
      `✅ Versión actual: ${versionInfo.version} (build ${versionInfo.buildNumber})`,
    );
...
router.post("/version", async (req, res) => {
  try {
    ...
    CURRENT_APP_VERSION.releaseNotes =
